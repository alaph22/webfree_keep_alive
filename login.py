...
def send_telegram_message(bot_token, chat_id, message):
    """使用 requests 向 Telegram Bot API 发送消息"""
...
def login_account(playwright, USER, PWD, max_retries: int = 2):
    ...
    while attempt <= max_retries:
        ...
        try:
            ...
            if any(s in html for s in failure_signs):
                ...
                if context: context.close()
                if browser: browser.close()
                # 更改：失败时应抛出异常，以便 main 捕获
                raise RuntimeError("登录失败(密码错误或账号无效)")
            ...
            if attempt <= max_retries:
                ...
                # 抛出异常，让主函数知道
                raise RuntimeError(f"账号 {USER} 所有 {max_retries + 1} 次尝试均已失败: {e}")
    ...
    raise RuntimeError(f"账号 {USER} 所有 {max_retries + 1} 次尝试均已失败")
...
def main():
    ...
    if not all([bot_token, chat_id, site_accounts]):
        log("❌ 缺少必要的环境变量 (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, 或 SITE_ACCOUNTS)")
        return
        ...
        with sync_playwright() as p:
            for user, pwd in accounts:
                try:
                    ...
                    report_lines.append(f"❌ 账号: `{user}` - 失败: {e}")
...
if __name__ == "__main__":
    # 当你运行 "python login.py" 时，将从这里开始执行
    main()
